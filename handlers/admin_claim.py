...
async def cmd_logout(message: Message, command: CommandObject):
    ...
    if (command.args or "").strip().lower() != "confirm":
        if owner:
            ...
            warn = (
                "⚠️ Ты последний владелец: бот перезапустится в режим привязки,\n"
        ...
        await message.answer(
            "Отвязать твой аккаунт от бота?\n\n" + warn + "Подтверждение: /logout confirm"
        )
        return
    if not owner:
        access.deny(uid)
        events.log("logout", user_id=uid, chat_id=message.chat.id, payload={"kind": "db-user"})
        await message.answer(
            "✅ Твой доступ снят.\n"
