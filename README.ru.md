...
- **Блокировка разрушительных команд.** Команды с катастрофическим радиусом
  (`rm -rf /`, `$HOME`, `~/.ssh`, запись в устройства) не выполняются: их отклоняет
  PreToolUse-хук у Claude Code и Qwen Code. Остальное только показывается
  предупреждением в чат. Это не песочница — её обходит любой чужой интерпретатор
  (`base64 -d | sh`, `python3 -c`); измеренные границы перечислены в SECURITY.md.
  Выключатель — `HEREASSISTANT_RISK_HOOK=0`.
