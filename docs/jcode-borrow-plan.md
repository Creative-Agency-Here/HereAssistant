...
| Пункт корзины 1 | Состояние |
|---|---|
| 1. Гарантированный cancel/kill/reap | выполнен 2026-07-29 (`820a7c8`) |
| 2. Fail-soft разбор native transcript | выполнен 2026-07-29 (`4eeeb0f`) |
| 3. Очередь следующего turn-а | выполнен 2026-07-29 (`820a7c8`) |
| 4. Resume-каталог Claude и Codex | выполнен 2026-07-29 (`cfbae35`) |
| 5. Детектор опасных shell-команд | выполнен 2026-07-29 (`b3993ad` + `7766013`) |

**Корзина 1 закрыта полностью.**

## Решения по корзине 2 (приняты 2026-07-29)

Решения выработаны при участии внешнего консультанта (Codex) и приняты по фактам,
проверенным на живой системе, а не по документации.

| Пункт | Решение | Основание |
|---|---|---|
| Блокирующий pre-tool approval | **Делаем, только Claude, только автоматически** | Зонд показал: `PreToolUse` работает в headless, блокирует вызов, отказ виден снаружи. Но при таймауте — fail-open, поэтому ждать человека внутри хука нельзя |
| Семантическая память на ONNX | **Не делаем сейчас** | Пользы не измерить: нет корпуса, на котором лексический поиск доказанно проигрывает. Плюс требует ручной офлайн-поставки модели |
| Векторы и confidence в SQLite | **Нет** | Следует из предыдущего |
| jcode пятым провайдером | **Нет** | Ходит в API подписки под видом официального CLI — риск для рабочих аккаунтов |
| Headless-логин через Telegram | **Нет** | У `codex login` headless-флагов нет вовсе; для остальных пришлось бы строить свой секретный контур |
| Auto-poke | **Нет** | Человек и так в чате; автономный расход подписки без надзора |
| Персист импортированных сессий | **Нет, read-only каталог** | Уже сделано так |
| Несколько агентов в одном репо | **Нет, очередь** | Уже сделано; у jcode блокировок файлов нет вовсе |
| PowerShell/cmd | **Помечать неподдерживаемым** | Правила bash к другой грамматике неприменимы |

Расхождение с консультантом зафиксировано честно: он рекомендовал не делать блокатор
вовсе («нет документированного контракта») и взять вместо него семантическую память.
Его аргумент опровергнут зондом уже после постановки задачи — контракт есть и проверен;
его же условие внедрения (fixture, доказывающий allow/deny/timeout) выполнено.
Обратное решение по памяти принято потому, что её польза остаётся гипотезой,
а блокировка разрушительных команд — измеримый результат.

## Что показала приёмка классификатора

Модуль `core/command_risk.py` написан CLI-агентом по спецификации из отчёта разбора.
Его собственные 57 тестов были зелёными, `ruff` и `pyright` — чистыми. Независимая
проверка набором из 26 команд, которых агент не видел, нашла две дыры:

1. `FOO=bar rm -rf /` возвращал `SAFE`. Присваивание переменной перед командой не
   снималось, разрушающий глагол вообще не находился. Опаснее всего тем, что это
   обычная форма записи, а не экзотика.
2. `rm -rf /var/lib` возвращал `CONFIRM`, тогда как `rm -rf /var/lib/docker` —
   `CATASTROPHIC`. Проверка «строго внутри защищённого каталога» пропускала сам
   каталог: удалить `/var/lib` целиком оценивалось мягче, чем удалить папку в нём.

Обе закрыты и покрыты регрессионными тестами. Вывод для последующих делегирований:
зелёные тесты агента не заменяют приёмку независимым набором кейсов.

**Отдельная грабля мутационной приёмки.** Первый прогон мутаций дал недостоверный
результат: цикл «испортить правило → прогнать тесты → откатить копией» уложился в одну
секунду, а размер файла совпал с исходным. Python считает `.pyc` валидным по паре
(mtime с точностью до секунды, размер), поэтому дальше исполнялся байткод мутанта, хотя
исходник был уже восстановлен. Перед каждым прогоном мутации нужно удалять
`core/__pycache__`, иначе приёмка проверяет не то, что лежит на диске.
