...
Команда `/resume` терминального чата показывает недавние сессии текущего проекта,
читая хранилище самого CLI (`core/session_import.py`, только чтение):

| Провайдер | Где лежат сессии | Как определяется проект |
|---|---|---|
| `claude_code` | `<auth home>/projects/<slug-от-cwd>/<id>.jsonl` | по имени каталога |
| `codex` | `<auth home>/sessions/ГГГГ/ММ/ДД/rollout-*.jsonl` | по полю `cwd` записи `session_meta` |

Заголовок берётся из первой реплики человека; служебные врезки CLI, вызовы
инструментов и рассуждения в список не попадают. Файл вне auth home аккаунта не
читается даже по symlink. Для `gemini` нативного resume нет — список пуст.
