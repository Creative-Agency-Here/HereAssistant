# Единый runtime Claude и Codex

## Цель

HereAssistant показывает один логический диалог, а Claude Code и Codex остаются сменными
...
определения. Это подтверждение нельзя обходить автоматической записью trust-state.
...
4. Оба провайдера получат один индекс и релевантные файлы общей памяти.
