...
async def run_git(*args: str, cwd: Path | None = None, timeout: int = 300) -> str:
...
async def clone_project(user_id: int, name: str, url: str) -> sqlite3.Row:
    ...
    try:
        await run_git("clone", "--", url, str(destination), timeout=600)
...
async def create_worktree(user_id: int, project_id: int, branch: str) -> sqlite3.Row:
    ...
    await run_git("worktree", "add", "-b", branch, str(destination), cwd=root)
    return projects.register_owned_project(user_id, f"{project['name']}--{branch}", destination)


async def status(root: str | Path) -> str:
    return await run_git("status", "--short", "--branch", cwd=Path(root))


async def pull(root: str | Path) -> str:
    return await run_git("pull", "--ff-only", cwd=Path(root), timeout=600)


async def push(root: str | Path) -> str:
    directory = Path(root)
    remotes = (await run_git("remote", cwd=directory)).splitlines()
    ...
    for remote in targets:
        output.append(await run_git("push", remote, "HEAD", cwd=directory, timeout=600))
