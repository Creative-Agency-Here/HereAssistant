...
- **Destructive-command guard** — commands with catastrophic blast radius (`rm -rf /`, `$HOME`, `~/.ssh`, writes to devices) are refused before they run via a PreToolUse hook for Claude Code and Qwen Code; everything else is only reported to the chat. Measured limits and what it deliberately does not catch are documented in [SECURITY.md](SECURITY.md).
