...
- **Аккаунты-подписки с владельцем** — `accounts.owner_user_id`; `NULL` означает
  «не назначен» и не даёт доступа. Общий профиль требует явного `shared=1`.
  Диалог берёт только СВОЙ аккаунт → явно shared → `ACCOUNT_NOT_AVAILABLE`;
  fallback на чужой enabled-профиль запрещён. Владелец/shared задаются в
  `manage.py`, видны в шапке.
- **Проекты** — доверенные roots живут в `projects`, общий доступ только через
  `visibility=shared` + `project_members`. Telegram `/cwd` разрешает лишь путь
  внутри выбранного root после `Path.resolve()`; произвольные пути запрещены.
  Локальный `chat.py` остаётся доверенным операторским TUI.
...
- **`CLI_TUI_STATUS.md`** — ⚠️ **ОБЯЗАТЕЛЬНО ПРОЧИТАЙ ПЕРЕД РАБОТОЙ НАД `cli/`**: текущее состояние HA CLI TUI, что работает, что сломано (стрелки, курсор, ESC из /resume), гипотезы и варианты решения. Баги подтверждены 2026-07-23, требуют интерактивной отладки.
